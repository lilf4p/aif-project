...
imageTest = image_test.ImageTest("images/bart.jpg", POLYGON_SIZE)
...
def saveImage(gen, polygonData):
    ...
    if gen % 100 == 0:
        ...
        folder = "images/results/run-{}-{}-{}-{}".format(POLYGON_SIZE, NUM_OF_POLYGONS,C_METHOD,start_time)
...
def main():
    ...
    folder = "images/results/run-{}-{}-{}-{}".format(POLYGON_SIZE, NUM_OF_POLYGONS, C_METHOD, start_time)
    # save gif
    imageTest.saveGif("result/grayscale/run-{}-{}-{}-{}-{}-{}/result.gif".format(POLYGON_SIZE, NUM_OF_POLYGONS, C_METHOD, POPULATION_SIZE, MAX_GENERATIONS, start_time))
    np.savetxt(folder + '/train_df.csv', logbook, delimiter=',', fmt='%s')
