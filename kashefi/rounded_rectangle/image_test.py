...
class ImageTest:
    ...
    def polygonDataToImage(self, polygonData):
        ...
        # chunkSize = self.polygonSize * 2  # (x,y) per vertex + (RGBA)
        ...
        for poly in polygons:
            ...
            arc_width = int(poly[index] * self.width)
            radios = int(poly[index] * self.width)
            # draw.arc(vertices, start, end, (red, green, blue, alpha), arc_width)
            draw.rounded_rectangle(vertices,radios, (red, green, blue, alpha), arc_width)
            # draw.arc(vertices, start, end, (gray, gray, gray, alpha), arc_width)
            # draw.arc(vertices, start, end, 127, arc_width)
            # draw.chord(vertices, start, end, (red, green, blue, alpha), arc_width)
    ...
    def get_random_angles(self):
        ...
        # end = (start + random.randint(1,360)) % 360 # a random size
        end = (start + 90) % 360 # a fixed size
        return start, end
