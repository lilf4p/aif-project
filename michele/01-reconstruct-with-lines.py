...
NUM_OF_PARAMS = NUM_OF_POLYGONS * (POLYGON_SIZE * 2 + NUM_COLORS)
...
imageTest = image_test_linest.ImageTest("images/Mona_Lisa_head.png", POLYGON_SIZE, True)
...
def randomFloat(low, up):
    if (POLYGON_SIZE == 2):
        a = [random.uniform(l, u) for l, u in zip([low] * NUM_OF_PARAMS, [up] * NUM_OF_PARAMS)]
        dMax = 0.4
        for i in range(0,NUM_OF_POLYGONS):
            p1x = random.uniform(low, up)
            p1y = random.uniform(low, up)
            p2x = p1x + random.uniform(-dMax, dMax)
            p2y = p1y + random.uniform(-dMax, dMax)
            if (p2x < 0.0):
                p2x = float(0.0)
            elif p2x > 1.0:
                p2x = float(1.0)
            if (p2y < 0.0):
                ...
                p2y = float(1.0)
            a[0+i*(POLYGON_SIZE * 2 + 4)]=p1x
            a[1+i*(POLYGON_SIZE * 2 + 4)]=p1y
            a[2+i*(POLYGON_SIZE * 2 + 4)]=p2x
            a[3+i*(POLYGON_SIZE * 2 + 4)]=p2y
        return a
    else:
        return [random.uniform(l, u) for l, u in zip([low] * NUM_OF_PARAMS, [up] * NUM_OF_PARAMS)]
...
# toolbox.register("select", tools.selRoulette, k=2)
