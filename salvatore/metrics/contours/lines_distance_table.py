...
class LinesNNPointContoursMetric(ContoursLineMetric):
    ...
    def __init__(self, image_path: str, canny_low: TReal, canny_high: TReal, bounds_low=0.0,
                 bounds_high=1.0, lineno: int = 500, results=None, point_adherence_coeff: float = 10.0,
                 line_adherence_coeff: float = 1.0, line_l1_lambda: float = 5.0):
        """
    ...
    def standardize_target(self):
        ...
        self.target_table = build_distance_table(self.image_height, self.image_width, self.num_targets, aux)
    ...
    def _core_get_difference(self, individual: TArray, index: int = 0):
        ...
        p, q = np.sum(aux), np.sum(np.abs(cv2.subtract(self.target_cv2, individual_img))) // 255
